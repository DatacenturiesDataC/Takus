...
describe('Edge Store (Knowledge Graph)', () => {
  beforeEach(async () => {
    ...
    const edges = await getEdgesForNode('recording', 'test-rec-1');
    ...
    const edges3 = await getEdgesForNode('recording', 'test-rec-2');
  ...
  it('adds and retrieves an edge', async () => {
    ...
    const edges = await getEdgesFromNode('recording', 'test-rec-1');
  ...
  it('removes all edges for a node', async () => {
    ...
    await removeEdgesForNode('recording', 'test-rec-1');
    const edges = await getEdgesForNode('recording', 'test-rec-1');
    expect(edges.length).toBe(0);
  });
